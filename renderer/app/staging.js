...
function updateHunkDecorations() {
    ...
    if (entry) {
        ...
        for (const k of [...entry.excluded])
            if (!valid.has(k))
                entry.excluded.delete(k);
    ...
    hunkDecorationIds = ed.deltaDecorations(hunkDecorationIds, changes.map((c) => {
...
function rebuildPartialEntry(path) {
    ...
    if (!entry || !hunkStagingActive() || state.current.path !== path)
        return;
...
function toggleHunk(c) {
    ...
    const k = hunkKey(c);
    if (entry.excluded.has(k))
        entry.excluded.delete(k);
    else
        entry.excluded.add(k);
...
function buildPartialContent(changes, excluded) {
    ...
    for (const c of changes) {
        ...
        while (oPos < oStart)
            out.push(oLines[oPos++ - 1]);
        if (excluded.has(hunkKey(c))) {
            while (oPos < oEndEx)
                out.push(oLines[oPos++ - 1]); // keep original
        }
        else {
