...
async function doCommit(alsoPush: boolean): Promise<void> {
  ...
  const message = $<HTMLTextAreaElement>('commit-message').value;
  const amend = $<HTMLInputElement>('amend-checkbox').checked;
  ...
  try {
    $<HTMLButtonElement>('btn-commit').disabled = true;
    $<HTMLButtonElement>('btn-commit-push').disabled = true;
    ...
    $<HTMLTextAreaElement>('commit-message').value = state.commitTemplate || '';
    updateSubjectLength();
    $<HTMLInputElement>('amend-checkbox').checked = false;
...
function rememberCommitMessage(message: string): void {
