...
let conflictLensProvider = null;
let conflictReparseTimer;
function ensureConflictEditor() {
    ...
    conflictLensProvider = {
        ...
        provideCodeLenses: (model) => {
            if (!state.conflict || model !== conflictModel)
            ...
            state.conflict.regions.forEach((r, i) => {
                ...
                lenses.push({ range, command: { id: 'diffier.conflict.ours', title: `Accept ${oursLabel}`, arguments: [i] } }, { range, command: { id: 'diffier.conflict.theirs', title: `Accept ${theirsLabel}`, arguments: [i] } }, { range, command: { id: 'diffier.conflict.both', title: 'Accept Both', arguments: [i] } });
            });
            return { lenses, dispose() { } };
        },
...
function parseConflictRegions(model) {
    ...
    for (let i = 1; i <= model.getLineCount(); i++) {
        ...
        if (l.startsWith('<<<<<<<'))
            cur = { start: i, base: 0, sep: 0, end: 0 };
...
function reparseConflicts() {
    if (!state.conflict || !conflictModel)
        return;
    ...
    if (conflictLensEmitter && conflictLensProvider)
        conflictLensEmitter.fire(conflictLensProvider);
    updateConflictCount();
}
...
function conflictRegionText(r, which) {
    ...
    const push = (from, to) => {
        for (let i = from; i <= to; i++)
            lines.push(conflictModel.getLineContent(i));
    };
    const oursEnd = (r.base || r.sep) - 1;
    if (which === 'ours' || which === 'both')
        push(r.start + 1, oursEnd);
    if (which === 'theirs' || which === 'both')
        push(r.sep + 1, r.end - 1);
    return lines;
}
...
function acceptAllConflicts(which) {
    ...
    for (let i = state.conflict.regions.length - 1; i >= 0; i--)
        acceptConflict(i, which);
}
function gotoConflict(delta) {
    if (!state.conflict || !state.conflict.regions.length)
        return;
    const regions = state.conflict.regions;
    const line = conflictEditor.getPosition()?.lineNumber ?? 0;
    let target;
    if (delta > 0)
        target = regions.find((r) => r.start > line) || regions[0];
    else
        target = [...regions].reverse().find((r) => r.start < line) || regions[regions.length - 1];
