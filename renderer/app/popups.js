...
function renderBranchList() {
    ...
    const addItem = (el, action) => {
        ...
        if (branchState.items.length === branchState.active)
    ...
    if (locals.length)
        list.appendChild(popupItem('Local branches', { section: true }));
    for (const b of locals) {
        ...
        addItem(el, () => {
            if (!b.current)
                checkoutBranch(b.name, false);
        });
    }
    if (remotes.length)
        list.appendChild(popupItem('Remote branches', { section: true }));
...
async function checkoutBranch(name, create) {
    ...
    try {
        ...
        if (create)
            ...
            await window.api.gitCheckout(name);
        statusMsg('');
...
function toggleMsgHistory() {
    if ($('msg-history-popup').classList.contains('hidden'))
        openMsgHistory();
    else
        closePopups();
}
...
function openRepoPopup() {
    ...
    if (recents.length)
        list.appendChild(popupItem('Recent repositories', { section: true }));
    for (const dir of recents) {
        ...
        el.addEventListener('click', async () => {
            closePopups();
            if (state.repo && state.repo.root === dir)
                return;
            try {
...
function openFileContextMenu(e, file) {
    ...
    menu.textContent = '';
    menu.appendChild(popupItem('Show History', { icon: '🕘', onClick: () => showFileHistory(file.path) }));
    menu.appendChild(popupItem('Show Blame', {
        icon: '👤',
        onClick: () => {
            if (!state.blameOn)
                toggleBlame();
    ...
    menu.appendChild(popupItem('Copy Path', {
        icon: '📋',
        onClick: () => {
            navigator.clipboard.writeText(file.path).catch(() => { });
        },
    }));
    menu.appendChild(popupItem('Reveal in Finder', {
        icon: '📁',
        onClick: () => {
