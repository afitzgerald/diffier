...
async function loadLog(reset) {
    if (!state.repo)
        return;
    if (!reset && state.log.loading)
        return;
    if (reset)
        resetLog(state.log.filePath);
...
function logGraphSvg(c) {
    ...
    const parts = [];
    const line = (x1, y1, x2, y2, color) => parts.push(`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="1.5"/>`);
    // Lines from the row above.
    g.before.forEach((h, i) => {
        if (h === null)
            return;
        if (h === c.hash)
            line(x(i), 0, x(g.col), mid, laneColor(i)); // into the dot
        else
            line(x(i), 0, x(i), mid, laneColor(i));
    });
    ...
    parts.push(`<circle cx="${x(g.col)}" cy="${mid}" r="3" fill="${laneColor(g.col)}" stroke="none"/>`);
...
function appendLogRows(batch) {
    ...
    for (const c of batch)
        frag.appendChild(buildLogRow(c));
...
function renderLog() {
    ...
    if (state.log.filePath)
        $('log-file-filter-label').textContent = '🕘 ' + state.log.filePath;
    if (!state.log.entries.length) {
...
function parseRefs(refs) {
    ...
    return refs
        ...
        if (r.startsWith('HEAD -> '))
            return { name: r.slice(8), head: true };
        if (r === 'HEAD')
            return { name: 'HEAD', head: true };
        if (r.startsWith('tag: '))
            return { name: '🏷 ' + r.slice(5), head: false };
...
async function selectLogEntry(c) {
    ...
    if (state.log.filePath) {
        const f = det.files.find((x) => x.path === state.log.filePath) ||
...
function renderLogDetails(det) {
    ...
    for (const f of det.files) {
        ...
        row.addEventListener('click', () => {
            for (const el of filesEl.querySelectorAll('.tree-row'))
...
$('log-list').addEventListener('scroll', () => {
    const el = $('log-list');
    if (state.log.done || state.log.loading)
        return;
