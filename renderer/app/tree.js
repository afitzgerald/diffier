...
function appendFileLabel(row, file, opts = {}) {
...
function renderTreeWindow(files) {
    ...
    for (let i = first; i < last; i++)
        treeEl.appendChild(buildRowEl(rows[i]));
...
function updateCommitCount() {
    ...
    if (partials.length)
        text += `, ${partials.length} partial`;
    if (skipped.length)
        text += `, ${skipped.length} empty`;
...
$('tree-filter').addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        ...
        treeEl.focus();
        const rows = fileRows();
        if (rows.length && !rows.some((r) => r.key === state.selectedKey))
