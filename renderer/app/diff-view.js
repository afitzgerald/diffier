...
async function openDiff(file, revealEnd) {
    ...
    if (state.current !== file || state.conflict)
        return; // user moved on while we loaded
...
function editableModel() {
    ...
    if (state.current && !state.readOnlyDiff && modifiedModel)
        return modifiedModel;
    return null;
}
async function autosaveIfDirty() {
    const model = editableModel();
    if (!state.dirty || !state.current || !model)
        return;
    try {
...
function focusEditor() {
    if (state.conflict && conflictEditor)
        conflictEditor.focus();
    else if (diffEditor && state.current)
