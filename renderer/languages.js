...
(function (global) {
    ...
    function detect(monaco, filePath, content) {
        ...
        for (const l of langs) {
            if (l.filenames && l.filenames.some((f) => f.toLowerCase() === lbase))
                return l.id;
        }
        if (FILENAME_ALIASES[lbase])
            return FILENAME_ALIASES[lbase];
        if (lbase.startsWith('.env'))
            return 'ini';
        if (ext) {
            for (const l of langs) {
                if (l.extensions && l.extensions.some((e) => e.toLowerCase() === ext))
                    return l.id;
            }
            if (EXTENSION_ALIASES[ext])
