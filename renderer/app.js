...
const monacoReady = new Promise((resolve) => {
  ...
  require(['vs/editor/editor.main'], () => {
    ...
    // Any keypress other than F7 (or a bare modifier, e.g. the Shift in
    // Shift+F7) disarms the "go to next file" prompt.
    diffEditor.getModifiedEditor().onKeyDown((e) => {
      const k = e.browserEvent.key;
      if (k !== 'F7' && !['Shift', 'Control', 'Alt', 'Meta'].includes(k)) {
        state.f7Armed = false;
...
function nextDifference() {
  ...
  if (next) {
    ...
    state.f7Armed = true;
    toast('Press F7 to go to the next file');
  }
}
...
function prevDifference() {
  ...
  if (prev) {
    ...
    toast('Press ⇧F7 to go to the previous file');
  }
}
...
function inCommitMessage() {
  return document.activeElement === $('commit-message');
}

function inMonaco() {
  return !!document.activeElement?.closest('#diff-editor');
}
...
window.addEventListener(
  ...
  (e) => {
    ...
    // F7 / Shift+F7 — next/previous difference (works everywhere)
    if (e.key === 'F7') {
      e.preventDefault();
      e.stopPropagation();
      if (e.shiftKey) prevDifference();
      else nextDifference();
      return;
    }

    // Cmd+Enter — commit; Alt+Cmd+Enter — commit and push (IntelliJ dialog keys)
    if (cmd && e.key === 'Enter') {
      e.preventDefault();
      doCommit(e.altKey);
      return;
    ...
    // Alt+Left/Right — previous/next changed file (outside text-editing contexts,
    // where Option+arrow must stay word navigation)
    if (e.altKey && !cmd && (e.key === 'ArrowRight' || e.key === 'ArrowLeft')) {
      if (!inCommitMessage() && !inMonaco()) {
        e.preventDefault();
    ...
    // Escape — back to the changes tree (IntelliJ: Esc returns to tool window)
    if (e.key === 'Escape') {
      state.f7Armed = false;
      state.shiftF7Armed = false;
      treeEl.focus();
    ...
    // Tree navigation
    if (document.activeElement === treeEl) {
      const row = selectedRow();
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        moveSelection(1);
      ...
      } else if (e.key === 'Enter' || (cmd && e.key === 'ArrowDown')) {
        e.preventDefault();
...
window.api.onMenu(async (id) => {
  switch (id) {
    case 'open-repo': return openRepoDialog();
    case 'save': return saveCurrent();
    case 'next-diff': return nextDifference();
    case 'prev-diff': return prevDifference();
    case 'next-file': return void selectFileByOffset(1);
    case 'prev-file': return void selectFileByOffset(-1, true);
    case 'focus-tree': return treeEl.focus();
    case 'commit':
      $('commit-message').focus();
      return;
    case 'push': return doPush();
    case 'rollback': return doRollback();
    case 'refresh': return refreshStatus();
    case 'toggle-panel': {
    ...
    case 'window-focus': return refreshStatus(true);
  }
});
